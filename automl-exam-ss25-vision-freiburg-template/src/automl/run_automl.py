...
def main():
    ...
    try:
        ...
        if config.get('enable_plotting', True):
            import os, glob
            from automl.visualization import plot_automl_results

            pattern = os.path.join(args.results_dir, "automl_results_*.json")
            ...
            if json_files:
                ...
                plot_automl_results(
                    latest_file,
                    save_dir=os.path.join(args.results_dir, "plots"),
                    format=config.get('plot_format', 'png'),
                    interactive=not args.no_plots
                )
            else:
                print("No results JSON file found for visualization; skipping plots.")
